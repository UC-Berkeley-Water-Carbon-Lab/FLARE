...
# 15. Plot helpers
# =========================================================
def get_largest_event_row(db_path):
    conn = sqlite3.connect(db_path)
    row = conn.execute("""
        SELECT *
        FROM fire_events
        ...
        LIMIT 1
    """).fetchone()
    colnames = [d[0] for d in conn.execute("SELECT * FROM fire_events LIMIT 1").description]
    conn.close()
...
def get_event_objects_df(db_path, lineage_id):
    import pandas as pd
    conn = sqlite3.connect(db_path)
    df = pd.read_sql("""
        SELECT *
        FROM fire_objects
        WHERE lineage_id = ?
        ORDER BY doy, orig_id
    """, conn, params=(int(lineage_id),))
    conn.close()
    return df


def plot_largest_event_overview(db_path, raster_shape=(1792, 1671)):
    import matplotlib.pyplot as plt
    import numpy as np

    largest = get_largest_event_row(db_path)
    if largest is None:
        print("No event found.")
        return None

    lineage_id = int(largest["lineage_id"])
    objs = get_event_objects_df(db_path, lineage_id)

    accum = np.zeros(raster_shape, dtype=np.uint16)

    for _, row in objs.iterrows():
        y0, x0 = int(row["bbox_y_min"]), int(row["bbox_x_min"])
        h, w = int(row["shape_h"]), int(row["shape_w"])
        mask = decompress_binary_mask(row["mask_blob"], (h, w))
        accum[y0:y0+h, x0:x0+w] += mask.astype(np.uint16)

    yy, xx = np.where(accum > 0)
    if len(yy) == 0:
        print("No pixels found for this event.")
        return largest
    ...
    pad = 20
    y0 = max(0, y0 - pad)
    y1 = min(raster_shape[0], y1 + pad)
    x0 = max(0, x0 - pad)
    x1 = min(raster_shape[1], x1 + pad)

    cropped = accum[y0:y1, x0:x1]

    plt.figure(figsize=(10, 10))
    plt.imshow(cropped, origin="lower")
    plt.title(
        f"Largest event overview\n"
        f"lineage_id={lineage_id}, "
        f"peak_object_pixels={largest['peak_object_pixels']}, "
        f"days={largest['start_doy']}-{largest['end_doy']}"
    )
    plt.colorbar(label="number of occupied object-days")
    plt.tight_layout()
    plt.show()
